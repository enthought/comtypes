...
# generated by 'xml2py'
# flags '..\tools\windows.xml -m comtypes -m comtypes.automation -w -r .
# *TypeLibEx -r .*TypeLib -o typeinfo.py'
...
if TYPE_CHECKING:
    from ctypes import _CData, _Pointer  # NOQA
    from typing import (
        Any,
        Callable,
        List,
...
class ITypeInfo(IUnknown):
    ...
    if TYPE_CHECKING:
        ...
        def GetDllEntry(self, memid, invkind):
            ...
            """
            Return the dll name, function name, and
            ordinal for a function and invkind.
            """
            raise
    ...
    def AddressOfMember(self, memid, invkind):  # NOQA
    ...
    def Bind(self, name, flags=0, lHashVal=0):
        ...
        if kind == DESCKIND_FUNCDESC:
            ...
            fd.__ref__ = weakref.ref(
                fd,
                lambda dead: ti.ReleaseFuncDesc(bindptr.lpfuncdesc)
            )
            return "function", fd
            ...
            vd.__ref__ = weakref.ref(
                vd,
                lambda dead: ti.ReleaseVarDesc(bindptr.lpvardesc)
            )
            return "variable", vd
...
if hasattr(_oleaut32, "LoadTypeLibEx"):
    ...
    def LoadTypeLibEx(szFile, regkind=REGKIND_NONE):  # NOQA
        # type: (str, int) -> ITypeLib
        """Load, and optionally register a type library file"""
        ptl = POINTER(ITypeLib)()
...
def UnRegisterTypeLib(libID, wVerMajor, wVerMinor, lcid=0, syskind=SYS_WIN32):
    ...
    return _oleaut32.UnRegisterTypeLib(byref(
        GUID(libID)),
        wVerMajor,
        wVerMinor,
        lcid,
...
def RegisterTypeLib(tlib, fullpath, helpdir=None):
    ...
    return _oleaut32.RegisterTypeLib(
        tlib,
        c_wchar_p(fullpath),
        c_wchar_p(helpdir)
    )
...
class tagTYPEATTR(Structure):
    ...
    def __repr__(self):
        return (
            "TYPEATTR(GUID=%s, typekind=%s, "
            "funcs=%s, vars=%s, impltypes=%s)" % (
                self.guid,
                self.typekind,
                self.cFuncs,
                self.cVars,
...
class tagFUNCDESC(Structure):
    ...
    def __repr__(self):
        return (
            "FUNCDESC(memid=%s, cParams=%s, cParamsOpt=%s, "
            "callconv=%s, invkind=%s, funckind=%s)" % (
                self.memid,
                self.cParams,
                self.cParamsOpt,
                self.callconv,
...
ITypeInfo._methods_ = [
    ...
    STDMETHOD(
        HRESULT,
        'GetNames',
        [MEMBERID, POINTER(BSTR), UINT, POINTER(UINT)]
    ),
    COMMETHOD(
    ...
    #     HRESULT,
    #     'GetIDsOfNames',
    #     [POINTER(LPOLESTR), UINT, POINTER(MEMBERID)]
    # ),
    # this one changed, to accept c_wchar_p array
    STDMETHOD(
        HRESULT,
        'GetIDsOfNames',
        [POINTER(c_wchar_p), UINT, POINTER(MEMBERID)]
    ),
    STDMETHOD(
        HRESULT,
        'Invoke',
    ...
    STDMETHOD(
        HRESULT,
        'AddressOfMember',
        [MEMBERID, INVOKEKIND, POINTER(PVOID)]
    ),
    COMMETHOD(
    ...
    COMMETHOD(
        [], None, 'ReleaseTypeAttr',
        (['in'], POINTER(TYPEATTR))
    ),
    COMMETHOD(
        [], None, 'ReleaseFuncDesc',
        (['in'], POINTER(FUNCDESC))
    ),
    COMMETHOD(
        [], None, 'ReleaseVarDesc',
...
ICreateTypeInfo._methods_ = [
    ...
    #     HRESULT,
    #     'AddRefTypeInfo',
    #     [POINTER(ITypeInfo), POINTER(HREFTYPE)]
    # ),
    COMMETHOD(
...
class IProvideClassInfo(IUnknown):
    ...
    _methods_ = [
        # Returns the ITypeInfo interface for
...
class IProvideClassInfo2(IProvideClassInfo):
    ...
    _methods_ = [
        # Returns the GUID for the object's
        # outgoing IID for its default event set.
        COMMETHOD(
            [], HRESULT, "GetGUID",
            (['in'], DWORD, "dwGuidKind"),
