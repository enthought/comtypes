...
class TypeLib(object):
    ...
    def compile(self):
        ...
        if not os.path.isfile(idl_path) or open(idl_path, "r").read() != code:
            ...
            os.system(r'call midl /nologo %s /tlb %s' % (idl_path, tlb_path))

        # Register the typelib...
        tlib = comtypes.typeinfo.LoadTypeLibEx(tlb_path,
                                               comtypes.typeinfo.REGKIND_REGISTER)
...
class MyServer(comtypes.CoClass, ConnectableObjectMixin):
    ...
    _outgoing_interfaces_ = [TestLib.IMyEventInterface]
