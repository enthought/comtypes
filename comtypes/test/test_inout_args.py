    ...
    def run_test(self):
        ...
        fixedfn = comtypes.instancemethod(
            _fix_inout_args(
                dummyFunction,
                tuple(self.argtypes),
                tuple(self.paramflags)),
            self, None)
        
        dummy_arguments = (argtype() for argtype, paramflags in self.zipped() if paramflags[0] & 1 == 1)
    ...
    def test_inout_args(self):
        ...
        testCases = [
            ...
            TestEntry(
                ...
                [POINTER(IUnknown), POINTER(IUnknown), POINTER(c_ulong), POINTER(c_wchar_p)],
                [(1, 'pValues'), (2, 'ppData'), (3, 'pdwOptimalWriteBufferSize'), (3, 'ppszCookie')],
            )
        ]
        ...
        for i, permuted_params in enumerate(permutations(params)):
            with self.subTest(f"Permutations example {i}"):
                TestEntry(
                    [p[0] for p in permuted_params],
                    [p[1:3] for p in permuted_params]
                ).run_test()
