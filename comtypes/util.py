...
def _calc_offset():
    ...
    if sizeof(PyCArgObject) != type(byref(c_int())).__basicsize__:  # NOQA
...
def byref_at(
    ...
    # 'offset' to it:
    _c_void_p_from_address(
        id(ref) + _byref_pointer_offset
    ).value += offset
    return ref
