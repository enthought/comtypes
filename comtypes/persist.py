...
class IPropertyBag(IUnknown):
    ...
    _idlflags_ = []
    _methods_ = [
        # XXX Note: According to MSDN, pVar and pErrorLog
        # are ['in', 'out'] parameters.
        #
        # XXX ctypes does NOT yet accept POINTER(IErrorLog) as 'out' parameter:
        # TypeError: 'out' parameter 3 must be a pointer
        # type, not POINTER(IErrorLog)
        COMMETHOD(
            [], HRESULT, 'Read',
            (['in'], WSTRING, 'pszPropName'),
...
from comtypes import COMObject  # NOQA
