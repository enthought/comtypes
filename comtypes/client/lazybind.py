...
class NamedProperty(object):
    ...
    def __getitem__(self, arg):
        ...
        if isinstance(arg, tuple):
            return self.disp._comobj._invoke(  # NOQA
                self.get.memid,
                self.get.invkind,
                0,
                *arg
    ...
    def __setitem__(self, name, value):
        ...
        if comtypes._is_object(value):  # NOQA
            descr = self.putref or self.put
        else:
            descr = self.put or self.putref

        if isinstance(name, tuple):
            self.disp._comobj._invoke(  # NOQA
                descr.memid,
                descr.invkind,
                0,
                *(name + (value,))
...
class Dispatch(object):
    ...
    def __eq__(self, other):
        return (
            isinstance(other, Dispatch) and
            self._comobj == other._comobj  # NOQA
        )
    ...
    def __getattr__(self, name):
        ...
        if descr.invkind == DISPATCH_PROPERTYGET:
            ...
            if descr.funckind == FUNC_DISPATCH:
                if descr.cParams == 0:
                    return self._comobj._invoke(  # NOQA
                        descr.memid,
                        descr.invkind,
                        0
                    )
                ...
                raise RuntimeError(
                    "funckind %d not yet implemented" % descr.funckind
                )
    ...
    def __iter__(self):
        punk = self._comobj._invoke(  # NOQA
            DISPID_NEWENUM,
            DISPATCH_METHOD | DISPATCH_PROPERTYGET,
            0
        )
