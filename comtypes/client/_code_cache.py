...
def _find_gen_dir():
    ...
    if not _is_writeable(gen_path):
        ...
        if ftype is None:
            ...
            path = _get_module_filename(sys.frozendllhandle)  # NOQA
        ...
        if not os.path.exists(gen_dir):
            logger.info(
                "Creating writeable comtypes cache directory: '%s'",
                gen_dir
            )
...
def _create_comtypes_gen_package():
    ...
    try:
        ...
        try:
            comtypes_path = os.path.abspath(
                os.path.join(comtypes.__path__[0], "gen")
            )

            if not os.path.isdir(comtypes_path):
                os.mkdir(comtypes_path)
                logger.info(
                    "Created comtypes.gen directory: '%s'",
                    comtypes_path
                )
            ...
            module = sys.modules["comtypes.gen"] = (
                types.ModuleType("comtypes.gen")
            )
            comtypes.gen = module
