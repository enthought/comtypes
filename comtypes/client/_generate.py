...
def GetModule(tlib):
    ...
    if isinstance(tlib, base_text_type):
        ...
        if not is_abs:
            # try to get path after loading, but
            # this only works if already registered
            pathname = tlbparser.get_tlib_filename(tlib)
            if pathname is None:
                logger.info(
                    "GetModule(%s): could not resolve to a filename",
                    tlib
                )
                pathname = tlib_string
        # if above path torture resulted in an absolute path,
        # then the file exists (at this point)!
    ...
    # try to get the friendly-name, if not,
    # returns the real typelib wrapper module
...
def _load_tlib(obj):
    ...
    if isinstance(obj, base_text_type):
        # in any case, attempt to load and if
        # tlib_string is not valid, then raise
    ...
    # obj is a sequence containing libid
        ...
        if not ver:  # case of version numbers are not containing
            with winreg.OpenKey(
                winreg.HKEY_CLASSES_ROOT, r"TypeLib\%s" % libid
            ) as key:
                ver = [
                    int(v, base=16) for v in winreg.EnumKey(key, 0).split(".")
                ]
    ...
    # obj is a COMObject implementation
        ...
        return typeinfo.LoadRegTypeLib(
            GUID(obj._reg_libid_),  # NOQA
            *obj._reg_version_  # NOQA
        )
